...
| CryptoKitties Core | `0x06012c8cf97BEaD5deAe237070F9587f8E7A266d` | 0.4.18 | ok | 4 |
| CryptoKitties SaleAuc | `0xb1690C08E213a35Ed9bAb7B318DE14420FB57d8C` | 0.4.18 | ok | 2 |
...
| KyberNetworkProxy | `0x818E6FECD516Ecc3849DAf6845e3EC868087B755` | 0.4.18 | ok | 1 |
| Gnosis Safe Singleton | `0xd9Db270c1B5E3Bd161E8c8503c55cEABeE709552` | 0.7.6 | ok | 1 |
| OpenSea Wyvern v2 | `0x7f268357A8c2552623316e2562D90e642bB538E5` | 0.4.26 | ok | 1 |
...
| Lido stETH | `0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84` | 0.4.24 | ok | 1 |
...
- ClockAuctionBase._bid(uint256,uint256) (KittyCore.sol#1162-1221) uses address.transfer() -- forwards a fixed 2300-gas stipend; an EVM gas repricing (e.g. Glamsterdam) can make it insufficient and break this path.  (KittyCore.sol:1162)
...
- ClockAuction.withdrawBalance() (KittyCore.sol#1396-1405) uses address.send() -- forwards a fixed 2300-gas stipend; an EVM gas repricing (e.g. Glamsterdam) can make it insufficient and break this path.  (KittyCore.sol:1396)
- KittyBreeding.giveBirth(uint256) (KittyCore.sol#1020-1063) uses address.send() -- forwards a fixed 2300-gas stipend; an EVM gas repricing (e.g. Glamsterdam) can make it insufficient and break this path.  (KittyCore.sol:1020)

### CryptoKitties SaleAuc  (`0xb1690C08E213a35Ed9bAb7B318DE14420FB57d8C`)
- ClockAuctionBase._bid(uint256,uint256) (SaleClockAuction.sol#165-224) uses address.transfer() -- forwards a fixed 2300-gas stipend; an EVM gas repricing (e.g. Glamsterdam) can make it insufficient and break this path.  (SaleClockAuction.sol:165)
- ClockAuction.withdrawBalance() (SaleClockAuction.sol#399-408) uses address.send() -- forwards a fixed 2300-gas stipend; an EVM gas repricing (e.g. Glamsterdam) can make it insufficient and break this path.  (SaleClockAuction.sol:399)
...
- UniERC20._uniDecode(IERC20,bytes4,bytes4) (AggregationRouterV5.sol#889-932) hardcodes gas (20000) on an external call; an EVM gas repricing (e.g. Glamsterdam) can make it insufficient and break this path.  (AggregationRouterV5.sol:889)

### KyberNetworkProxy  (`0x818E6FECD516Ecc3849DAf6845e3EC868087B755`)
- Withdrawable.withdrawEther(uint256,address) (KyberNetworkProxy.sol#320-323) uses address.transfer() -- forwards a fixed 2300-gas stipend; an EVM gas repricing (e.g. Glamsterdam) can make it insufficient and break this path.  (KyberNetworkProxy.sol:320)
...
### OpenSea Wyvern v2  (`0x7f268357A8c2552623316e2562D90e642bB538E5`)
- ExchangeCore.executeFundsTransfer(ExchangeCore.Order,ExchangeCore.Order) (WyvernExchangeWithBulkCancellations.sol#912-1051) uses address.transfer() -- forwards a fixed 2300-gas stipend; an EVM gas repricing (e.g. Glamsterdam) can make it insufficient and break this path.  (WyvernExchangeWithBulkCancellations.sol:912)

### Lido stETH  (`0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84`)  -- via implementation `0x6ca84080381e43938476814be61b779a8bb6a600`
- VaultRecoverable.transferToVault(address) (@aragon/os/contracts/common/VaultRecoverable.sol#26-42) uses address.transfer() -- forwards a fixed 2300-gas stipend; an EVM gas repricing (e.g. Glamsterdam) can make it insufficient and break this path.  (@aragon/os/contracts/common/VaultRecoverable.sol:26)
